...
bool CheckFinalStatus(RiverBank &LeftNow,
                     RiverBank &RightNow); /* 检查所以东西是否已经运抵右岸 */
bool CheckForSafety(
    RiverBank &side); /* 检查当前左右两岸是否安全（农民不在时不能出现食物链） */
...
void PrintResultSet() {
  ...
  for (iter = ResultSet.begin(); iter != ResultSet.end(); iter++) {
    ...
    std::string LeftString =
        "Left: { " +
        (((iter->StateMaks) & WOLF) ? ObjectToString(WOLF) + " " : "") +
...
inline bool CheckFinalStatus(RiverBank &LeftNow, RiverBank &RightNow) {
  /* 所以东西都已经运抵右岸，包括农民自己 */
  if (LeftNow.Empty() && RightNow.Find(WOLF) && RightNow.Find(SHEEP) &&
      RightNow.Find(FARMER) && RightNow.Find(CABBAGE))
    return true;
  return false;
}
...
inline bool CheckForSafety(RiverBank &side) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if ((FoodChain.count(obj) != 0 && side.Find(FoodChain[obj]) &&
         side.Find(FARMER) == false) ||

        (ReverseFoodChain.count(obj) != 0 && side.Find(ReverseFoodChain[obj]) &&
         side.Find(FARMER) == false))
      return false;
  }
  return true;
  ...
  if (dir == TO_RIGHT) { /* 当前是从左岸去右岸 */
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (CheckForSafety(TempLeft) && CheckForSafety(TempRight)) {
        ...
        ResultSet.push_back(
            ResultItem{TO_RIGHT, obj, TempLeft.GetMask()}); /* 记录当前操作 */
        State.insert(TempLeft.GetMask());                   /* 记录状态 */
        DFS(TO_LEFT, TempLeft, TempRight); /* 基于当前状态搜索 */
        State.erase(
            TempLeft.GetMask()); /* 当前操作已经搜索过，清除状态，消除影响 */
        ResultSet
            .pop_back(); /* 当前操作已经搜索过，从结果集合中删掉，消除影响 */
